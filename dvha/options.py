...
class DefaultOptions:
    ...
    def __init__(self):
        ...
        self.ROI_TYPES = ['NONE', 'ORGAN', 'PTV', 'ITV', 'CTV', 'GTV',
                          ...
                          'IRRAD_VOLUME', 'REGISTRATION', 'TREATED_VOLUME', 'IGNORED']
...
class Options(DefaultOptions):
    ...
    def upgrade_options(self, loaded_options):
        ...
        # New options using a new attribute name will be automatically generated by the DefaultOptions class
        self.db_group_upgrade(loaded_options)
        self.dvh_selection_upgrade(loaded_options)
        self.roi_type_add_none(loaded_options)
    ...
    def dvh_selection_upgrade(self, loaded_options):
        ...
        # The following keys need updates to look nice, user can always change later
        if 'DVH_LINE_WIDTH_SELECTION' not in list(loaded_options):
            keys = ['IQR_ALPHA', 'STATS_MEDIAN_LINE_WIDTH', 'STATS_MEAN_LINE_WIDTH',
                    'STATS_MAX_LINE_WIDTH', 'STATS_MIN_LINE_WIDTH']
            for key in keys:
                loaded_options[key] = getattr(self, key)

    @staticmethod
    def roi_type_add_none(loaded_options):
        for roi_type in ['NONE', 'IGNORED']:
            if roi_type not in loaded_options['ROI_TYPES']:
                loaded_options['ROI_TYPES'].insert(0, roi_type)
