...
class Plan:
    ...
    def __init__(self, rt_plan_file, **kwargs):
        ...
        self.tps = '"%s %s"' % (
            rt_plan.Manufacturer,
            rt_plan.ManufacturerModelName,
        )

        self.complexity_scores = [
            float(fx_grp.complexity_score) for fx_grp in self.fx_group
        ]
    ...
    def __str__(self):
        summary = [
            ...
            % ", ".join(["%0.3f" % cs for cs in self.complexity_scores]),
        ]
        return "\n".join(summary)
...
class FxGroup:
    ...
    def __init__(self, fx_grp_seq, plan_beam_sequences, **kwargs):
        ...
        self.complexity_score = np.sum(
            np.array([np.sum(beam.complexity_scores) for beam in self.beam])
        )
...
class Beam:
    ...
    def __init__(
        ...
        self.control_point_meter_set = np.append(
            [0], np.diff(np.array([cp.cum_mu for cp in self.control_point]))
        )
        ...
        x_paths = np.array(
            [get_xy_path_lengths(cp)[0] for cp in self.aperture]
        )
        y_paths = np.array(
            [get_xy_path_lengths(cp)[1] for cp in self.aperture]
        )
        ...
        self.complexity_scores = (
            np.divide(
                np.multiply(np.add(c1 * x_paths, c2 * y_paths), cp_mu), area
            )
            / self.meter_set
        )
        ...
        self.summary = {
            ...
            "area": np.divide(area, 100.0).tolist(),
            "x_perim": np.divide(x_paths, 10.0).tolist(),
            "y_perim": np.divide(y_paths, 10.0).tolist(),
            "perim": np.divide(np.add(x_paths, y_paths), 10.0).tolist(),
...
class ControlPoint:
    ...
    def __init__(self, cp_seq, leaf_boundaries, **kwargs):
        ...
        if hasattr(cp_seq, "BeamLimitingDevicePositionSequence"):
            for (
                device_position_seq
            ) in cp_seq.BeamLimitingDevicePositionSequence:
                if hasattr(
