...
class ExportFigure(wx.Frame):
    def __init__(self, parent):
        wx.Frame.__init__(self, parent, style=wx.SYSTEM_MENU | wx.CLOSE_BOX | wx.CAPTION)
    ...
    def __do_bind(self):
        ...
        self.Bind(wx.EVT_CHECKBOX, self.on_checkbox, id=self.include_range.GetId())
    ...
    def __do_layout(self):
        ...
        for obj_type in ['figure', 'legend']:
            for i, input_obj in enumerate(self.input[obj_type]):
                sizer_input[obj_type].Add(self.label[obj_type][i], 0, wx.EXPAND | wx.LEFT | wx.RIGHT | wx.TOP, 3)
                sizer_input[obj_type].Add(input_obj, 0, wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, 3)
        ...
        sizer_buttons.Add(self.button['Export'], 1, wx.EXPAND | wx.ALIGN_CENTER | wx.ALL, 5)
        sizer_buttons.Add(self.button['Dismiss'], 1, wx.EXPAND | wx.ALIGN_CENTER | wx.ALL, 5)
        sizer_main.Add(sizer_buttons, 1, wx.EXPAND | wx.ALIGN_CENTER | wx.ALL, 5)
    ...
    def get_attr_dict(self, obj_type, save_mode=False):
        return {key: self.getter[type(value)](obj_type + '_' + key)
                ...
                if save_mode or ('range' not in key or ('range' in key and self.include_range.GetValue()))}
